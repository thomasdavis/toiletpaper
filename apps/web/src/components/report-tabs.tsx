  ...
  return (
    <div className={`rounded-lg border ${colors.border} ${colors.bg} overflow-hidden`}>
      ...
      <button
        ...
        className="flex w-full items-start gap-3 p-4 text-left transition-colors hover:bg-white/40"
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          {claim.simulations.length > 0 && claim.simulations[0].reason && (
            <p className={`mt-1 text-xs ${colors.text}`}>
              {claim.simulations[0].reason.slice(0, 150)}
              {(claim.simulations[0].reason.length ?? 0) > 150 ? "…" : ""}
            </p>
          )}
        </div>
        <div className="flex shrink-0 items-center gap-2">
          ...
          <span className="text-[#9B9B9B]">{expanded ? "▲" : "▼"}</span>
        </div>
      </button>

      {/* Expanded details */}
      {expanded && claim.simulations.length > 0 && (
        <div className="border-t border-[#E8E5DE] bg-white p-4 space-y-4">
          {claim.simulations.map((sim, i) => (
            <div key={i} className="space-y-3">
              ...
              {sim.reason && (
                <div className="rounded border border-[#E8E5DE] bg-[#FAFAF8] p-3">
